...
if (isset($_POST['add_to_cart'])) {
    ...
    // Fetch the stock availability for the selected product
    $check_stock = mysqli_query($conn, "SELECT stock FROM `products` WHERE id = '$product_id'") or die('query failed');
    ...
    if ($fetch_stock['stock'] < $product_quantity) {
        ...
        $message[] = 'Insufficient stock available for ' . $product_name;
    } else {
        $check_cart_numbers = mysqli_query($conn, "SELECT * FROM `cart` WHERE name = '$product_name' AND user_id = '$user_id'") or die('query failed');
        ...
        if (mysqli_num_rows($check_cart_numbers) > 0) {
            ...
            // Deduct the booked quantity from the stock
...
<section class="packages">
   ...
   <div class="box-container">
      ...
      ?>
      <form action="" method="POST" class="box">
         <a href="view_page.php?pid=<?php echo $package['package_id']; ?>" class="fas fa-eye"></a>
